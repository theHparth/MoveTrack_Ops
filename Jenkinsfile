pipeline {
    ...
    stages {
        ...
        stage('Trivy Scan') {
            steps {
                sh 'docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v movetrack_ops_jenkins_home:/report aquasec/trivy:latest image -f json -o /report/trivy-report.json shipment-ingestion'
